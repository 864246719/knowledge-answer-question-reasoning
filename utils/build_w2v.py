...
def write_jiebatxt(path):
    lines= []
    lines = read_lines(path)
    fileTrainSeg=[]
    for i in range(len(lines)):
        fileTrainSeg.append([' '.join(list(jieba.cut(lines[i][9:-11],cut_all=False)))])
        if i % 100 == 0:
            print(i)
    return fileTrainSeg
...
if __name__ == '__main__':
    build('{}/datasets/train_set.seg_x.txt'.format(BASE_DIR),
          ...
          '{}/datasets/sentences.txt'.format(BASE_DIR),
          '{}/datasets/jiebaoutput.txt'.format(BASE_DIR),
